<?php
	...
	function validateSiretEntreprise($siret_entreprise) {
		$reponse = mysql_query('SELECT siret FROM `entreprise` WHERE siret="'.$siret_entreprise.'"') or die(mysql_error());
	
		if(mysql_numrows($reponse) == 0 && preg_match("#[0-9]{14}#", $siret_entreprise)) { // valide
			return true;
		} else {
			return false;
		}
